...
const ProductRow = React.memo(function ProductRow({
	...
	item: ProductInventory & { quantity: number };
	isSelecting: boolean;
	isSelected: boolean;
			...
			<View style={[styles.card, { width: "100%", position: "relative" }]}>
				...
				>
					...
					<View style={{ flexShrink: 1 }}>
						...
						<Text style={styles.detail}>Đơn vị: {item.unit}</Text>
...
/* --------------------------- Custom hooks --------------------------- */
function useInvoiceTotals(items: (ProductInventory & { quantity?: number })[]) {
	...
	const totalQuantity = useMemo(
		() => safeItems.reduce((sum, it) => sum + (it.quantity || 0), 0),
		[safeItems]
	);

	return { safeItems, totalQuantity };
}
...
export default function PaymentInvoiceScreen() {
	...
	// derived totals hook
	const { safeItems, totalQuantity } = useInvoiceTotals(items);

	const totalTaxDiscount = useMemo(
		() => (selectedTax !== null ? (totalQuantity * selectedTax) / 100 : 0),
		[selectedTax, totalQuantity]
	);
	const isPayDisabled = useMemo(() => totalQuantity === 0, [totalQuantity]);
	...
	useEffect(() => {
		setInvoiceDetail((prev: any) => ({
			...prev,
			items: initialItems,
			total: totalAfterTax ?? totalQuantity,
			tax: selectedTax ?? 0,
			note: description,
		}));
	}, [items, totalAfterTax, totalQuantity, selectedTax, description]);
	...
	const changeQuantity = useCallback((id: string, raw: string) => {
		...
		setItems((prev) =>
			prev.map((it) => (it._id === id ? { ...it, quantity: n } : it))
		);
	}, []);
	...
	const handleSaveTax = useCallback(() => {
		...
		setTotalAfterTax(Math.max(totalQuantity - discount, 0));
		// close animations
	...
	}, [selectedTax, totalTaxDiscount, totalQuantity, animatedHeight]);

	// ensure totalAfterTax updates when items or tax change
	useEffect(() => {
		if (selectedTax !== null) {
			setTotalAfterTax(
				Math.max(totalQuantity - (totalQuantity * selectedTax) / 100, 0)
			);
		} else {
			setTotalAfterTax(null);
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [totalQuantity, selectedTax]);

	// FlatList optimizations
	const keyExtractor = useCallback((item: ProductInventory) => item._id, []);

	const renderItem = useCallback(
		({ item }: { item: ProductInventory & { quantity: number } }) => (
			<ProductRow
				item={item}
                  ...
                  >
                    {readNumber(totalPrice)} đồng
								...
								>
									...
									>
										{totalAfterTax ?? totalQuantity} sản phẩm
									</Text>
								</View>
							</View>
