...
import { NewProductInventory, ProductInventory } from "@/src/types/storage";
...
function ModalCreateProductsByInvoiceInput({
	...
	const toggleSelect = (item: InvoiceProduct) => {
		setSelectedProducts((prev) => {
			if (prev.includes(item.ten)) {
				return prev.filter((name) => name !== item.ten);
			} else {
				return [...prev, item.ten];
			}
		});
	};
	const handleCreateMultiple = async () => {
		...
		try {
			...
			// Lọc sản phẩm từ hóa đơn theo tên
			const itemsToCreate = productSynchronized.filter((p) =>
				selectedNames.includes(p.ten)
			);

			for (const item of itemsToCreate) {
				const newProduct: NewProductInventory = {
					name: item.ten,
					unit: item.dvtinh,
					stock: 0,
					price: Number(item.dgia),
					imageURL: item.id ?? "",
					category: item.tchat ?? "",
					code:
						item.stt !== undefined && item.stt !== null
							? item.stt.toString()
							: "",
					description: item.tchat ?? "",
					attributes: [],
				};
				await createProductInventory(newProduct);
				Alert.alert("Thành công", "Đã thêm nguyên liệu vào kho");
			}

			setSelectedProducts([]); // clear
		} catch (error: any) {
			const errorMessage = error?.message || "Không thể thêm nguyên liệu";
	...
	const renderItem = ({ item }: any) => {
		const isSelected = selectedProducts.includes(item.ten);
		...
		return (
			<TouchableOpacity onPress={() => toggleSelect(item)}>
				...
				>
					<View style={{ position: "absolute", right: 5, top: 5, flex: 1 }}>
							...
							>
								...
								>
									<MaterialCommunityIcons
										...
										color={ColorMain}
									/>
									<Text style={{ textAlign: "right", color: ColorMain }}>
										Chọn tất cả
									</Text>
								</TouchableOpacity>
							</View>
...
export default ModalCreateProductsByInvoiceInput;
